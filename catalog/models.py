...
class BookTransport(models.Model):
    STATUS_CHOICES = [
        ('P', 'Pending'),
        ('A', 'Accepted'),
        ('R', 'Rejected'),
    ...
    booking_date = models.DateField()
    num_cars = models.PositiveIntegerField(default=1)  # Number of cars booked
    total_price = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    status = models.CharField(max_length=1, choices=STATUS_CHOICES, default='P')

    def calculate_total_price(self):
        # Calculate total price based on the number of cars and price per car
        return self.num_cars * float(self.transport.price)
    ...
    def __str__(self):
        return f"Booking for {self.transport.service_name}"